...
META=$(curl -fsSL \
  ...
  https://api.github.com/repos/abhigyanpatwari/GitNexus/releases/latest)
...
if [[
  ...
  && "$CURRENT_NODE_MODULES_HASH" != "sha256-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="
]]; then
...
PREFETCH_JSON=$(nix-prefetch-github abhigyanpatwari GitNexus --rev "v$VERSION" --json)
HASH=$(jq -r '.hash' <<< "$PREFETCH_JSON")

echo "Hash: $HASH"
echo "Computing node_modules hash (fixed-output derivation)"

# Write version + source hash first so node_modules derivation is evaluated
# against the right GitNexus release when we force a fake fixed-output hash.
jq -n --arg v "$VERSION" --arg hash "$HASH" --arg nmHash "sha256-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=" \
...
BUILD_LOG=$(
  nix build --impure --no-link --expr "
    ...
    in
      pkg.passthru.node_modules
  " 2>&1 >/dev/null || true
)

NODE_MODULES_HASH=$(sed -n 's/^[[:space:]]*got:[[:space:]]*//p' <<< "$BUILD_LOG" | sed -n '1p')
